...
begin
  ...
  try
    ...
    Objeto := dm.RESTResponse1.JSONValue as TJSONObject;
      ...
      Result     :=  true;
      lagradouro := RemoveAcento(Objeto.GetValue('logradouro').Value);
      bairro     := RemoveAcento(Objeto.GetValue('bairro').Value);
      localidade := RemoveAcento(Objeto.GetValue('localidade').Value);
